...
$S3_BUCKET = "sant-sumun-dev"  # Altere para seu bucket
$LAMBDA_FUNCTION = "sumun-preprocess-columns"  # Altere para sua função Lambda
$STACK_NAME = "sumun-prompt-tester-stack"
$ECR_REPO_NAME = "sumun-llm-teste-backend"
...
$ecrExists = aws ecr describe-repositories --repository-names $ECR_REPO_NAME --region $REGION 2>$null
if (!$ecrExists) {
...
sam deploy `
  ...
  --region $REGION `
  --parameter-overrides S3BucketName=$S3_BUCKET LambdaProcessingFunctionName=$LAMBDA_FUNCTION `
...
# Step 6: Obter URL da API e atualizar frontend
...
# Step 7: Atualizar arquivo de configuração do frontend
...
# Step 9: Obter nome do bucket do frontend
Write-Host "Step 9: Obtendo configurações da stack..." -ForegroundColor Yellow
...
# Step 10: Upload do Frontend para S3
Write-Host "Step 10: Upload do frontend para S3..." -ForegroundColor Yellow
aws s3 sync frontend/dist s3://$FRONTEND_BUCKET --delete
Write-Host "  ✓ Frontend enviado para S3" -ForegroundColor Green
Write-Host ""

# Step 11: Invalidar cache do CloudFront
Write-Host "Step 11: Invalidando cache do CloudFront..." -ForegroundColor Yellow
# Pular invalidação do CloudFront por enquanto (pode ser feito manualmente se necessário)
Write-Host "  ⚠ Invalidação do CloudFront pulada (o cache será limpo automaticamente em alguns minutos)" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "  2. Configure as variáveis de ambiente se necessário" -ForegroundColor Gray
